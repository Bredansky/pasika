...
function printJson(bundle: MissingFileBundle | ReviewBundle | ReverseReviewBundle | UntrackedDocBundle): void {
...
function isDocsForResult(value: unknown): value is DocsForResult {
  ...
  return "file" in value && typeof value.file === "string" && "docs" in value && Array.isArray(value.docs);
}
...
function runTargetsFor(docPath: string, projectRoot: string): TargetsForResult {
  ...
  if (result.status !== 0) {
    ...
    const message = stderr || stdout || `Failed to resolve tracked targets for ${docPath}.`;
...
function buildMissingFileBundle(inputPath: string, absolutePath: string): MissingFileBundle {
  return {
    mode: "file",
    file: {
      inputPath,
    ...
    recommended_actions: [
      {
        kind: "review_scope",
        target: inputPath,
        reason: "Confirm the intended current file path before attempting a doc drift review.",
      },
    ],
    metadata: {
      docsForCommand: null,
...
function buildFileReviewBundle(inputPath: string, projectRoot: string): MissingFileBundle | ReviewBundle {
  ...
  return {
    ...
    metadata: {
      ...
      externalSyncMissing: docs.some((doc) => doc.kind === "external" && doc.availability === "missing_synced_copy"),
      missingDocs: docs
...
function buildDocReviewBundle(inputPath: string, projectRoot: string): ReverseReviewBundle | UntrackedDocBundle {
  const absolutePath = path.resolve(projectRoot, inputPath);
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Unknown error while resolving doc targets.";
    ...
    return {
      ...
      status: "insufficient_evidence",
      confidence: "low",
      summary: "The requested doc is not tracked by vulyk, so reverse doc-to-target review cannot be resolved.",
      findings: [
        {
          type: "untracked_doc",
          severity: "medium",
      ...
      if (target.kind === "directory") {
        return {
          ...
          review: {
            ...
            status: "insufficient_evidence",
            confidence: "low",
            summary: "This doc maps to a directory target, which is too broad for automatic single-file drift review.",
            findings: [
              {
                type: "insufficient_specificity",
                severity: "medium",
...
function main(): void {
  ...
  if (args.length === 0) {
    failUsage("Usage: npx tsx .claude/skills/doc-drift-review/scripts/review-doc-drift.ts <file> | --doc <doc>");
  }

  if (args[0] === "--doc") {
    const inputPath = args[1];
    if (!inputPath) {
      failUsage("Usage: npx tsx .claude/skills/doc-drift-review/scripts/review-doc-drift.ts --doc <doc>");
    }
  ...
  if (!inputPath) {
    failUsage("Usage: npx tsx .claude/skills/doc-drift-review/scripts/review-doc-drift.ts <file>");
  }
