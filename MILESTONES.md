...
| 2026-08-27 | Found and fixed a zirka styleguide bug that made the CSS/Markdown/JSON language blocks dead code for every consumer: `loadPasikaConfigs` began with a global ignore (`**/*.css`, `package.json`, `**/*.md`) that overrides all later `files`-scoped blocks in flat config. Fixed by scoping the JS/TS base configs (node/browser/typescript/react/next/playwright) to JS/TS files instead; released zirka v0.0.42. This is why `eslint.docs.config.ts` existed — the styleguide's markdown block never fired. |
| 2026-08-27 | Removed the docs workaround: deleted `eslint.docs.config.ts`, `docs` script is plain `eslint docs`, and `eslint .` now covers docs. README.md/MILESTONES.md added to pasika's styleguide ignores (repo meta, not doc-guide docs). zirka dep bumped to `^0.0.42`; all gates green on clean `npm ci`; CI green. |
