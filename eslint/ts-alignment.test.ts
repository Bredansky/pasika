...
void describe("pasika TypeScript alignment diagnostic", () => {
  void it("accepts the same major (minor/patch drift is safe: the TypeFlags layout is stable within a major)", () => {
    assert.equal(alignmentError("6.0.3", "6.1.0"), null);
    assert.equal(alignmentError("6.1.2", "6.0.3"), null);
  });

  void it("rejects an older consumer major, urging an upgrade to pasika's bundled major", () => {
  ...
  void it("rejects a newer consumer major, urging alignment for now", () => {
  ...
  void it("skips when either copy cannot be resolved — never blocks an undeterminable setup", () => {
  ...
  void it("does not throw when resolved from inside pasika itself (bundled == hoisted)", () => {
