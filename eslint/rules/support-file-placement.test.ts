...
void describe("A constant MAY live in src/config/<config-name>/ instead of a constants/ folder when a developer determines that it configures application behavior and is best understood alongside the configuration that parameterizes it, even when consumers exist outside the config module.", () => {
  ruleTester.run("support-file-placement", supportFilePlacementRule, {
...
void describe("A type MAY stay in src/config/<config-name>/ when its meaning is derived from the configuration that it parameterizes, even when consumers exist outside the config module.", () => {
  ruleTester.run("support-file-placement", supportFilePlacementRule, {
