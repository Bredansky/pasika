...
const GLOSSARY = [
  ...
  "## Route Terms",
  "",
  "These terms name the parts of a route's error handling.",
  "",
  "| Term           | Definition                                     |",
  "| -------------- | ---------------------------------------------- |",
...
void describe("A How To section whose steps use terms that a glossary Reference defines MUST link that Reference from the section's first step.", () => {
  documentationRuleTester.run("glossary-term-linking", glossaryTermLinkingRule, {
    valid: [
      ...
      {
        // Pointing the read at the group the terms live in still satisfies it.
        filename: guideFile,
        code: [
          "# Example Guide",
          "",
          ...
          "Use this when adding a route handler.",
          "",
          "1. Read the Route Terms in the [Glossary Reference](references/glossary-reference.md#route-terms) so the wrapper has a name.",
          "2. Throw an `HttpError` from the delegated module.",
        ].join("\n"),
      },
      {
