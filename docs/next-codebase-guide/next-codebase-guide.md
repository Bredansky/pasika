...
4. Follow the [Route Handler Rule](rules/route-handler-rule.md) so the handler is wrapped in `withResponse` and never returns a constructed `HttpError` instead of throwing it.
