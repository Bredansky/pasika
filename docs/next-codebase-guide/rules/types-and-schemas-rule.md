...
- A `types/` or `schemas/` folder with one support file MUST define its exports directly in `index.ts`; with several support files, it MUST group related types and schemas in files that `index.ts` named-re-exports.
