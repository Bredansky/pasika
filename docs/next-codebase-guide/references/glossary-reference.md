...
| Imperative category          | A built-in React hook call, or one of four kinds of side-effecting work a hook can do: subscribing, external I/O, DOM manipulation, or resource lifecycle. The first one found is free; each one after that adds one to the extraction score. |
