...
| `withResponse`   | The wrapper that awaits a route handler's result — the `{ message, data }` envelope, or the `{ body, status, headers }` a JSON response cannot hold — and turns it, or a thrown `HttpError`, into a response.        |
| `HttpError`      | The error type a failed call throws, carrying the status and message the route responds with, and `data` — what a failed upstream call answered with; a module that needs a failure with its own name subclasses it. |
