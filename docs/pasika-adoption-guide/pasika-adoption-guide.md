...
10. Run `npx vulyk init`, `npx vulyk add`, and `npx vulyk agents` per the [Vulyk Docs Rule](rules/vulyk-docs-rule.md) so the repository's `vulyk.config.ts` tracks the framework's docs from `pasika` and the agent files that route to them are generated.
