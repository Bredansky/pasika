...
6. If the repository is a Next.js application, define the route error helpers per the [With Response Helper Rule](rules/with-response-helper-rule.md) so a failure a route reports becomes a response at the status it carries.
7. If the repository makes outbound HTTP requests, define the `zodFetch` helper per the [Zod Fetch Helper Rule](rules/zod-fetch-helper-rule.md) so every request hands back validated data and every failure keeps the status the upstream reported.
