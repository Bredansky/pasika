...
- `vulyk` MUST run as an ephemeral command such as `npx github:Bredansky/vulyk`.
- `vulyk` MUST NOT be added to `package.json` in order to run its CLI.
- A user MAY discover available commands with `npx github:Bredansky/vulyk --help` and command-specific usage with `npx github:Bredansky/vulyk <command> --help`.
