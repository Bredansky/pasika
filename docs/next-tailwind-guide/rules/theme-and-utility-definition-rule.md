...
- A custom utility that owns a combination of two or more styles MUST use the `*-skin` suffix.
- A repeated combination containing at least two semantic canvas, ink, or edge utilities plus any related styles MUST become a `*-skin` custom Tailwind utility that owns the combination.
...
@utility primary-skin {
  @apply bg-(--primary-canvas) text-(--primary-ink);
}
```

```tsx
<button className="primary-skin">Save</button>
<button className="primary-skin">Continue</button>
```

Why: canvas and ink CSS variables are used only by `primary-skin`, and the repeated combination has one named owner.

## Incorrect — Repeated Control Styles Have No Skin
...
## Correct — Control Skin Combines Edge and Canvas

```css
@utility control-skin {
  @apply border-control-edge bg-control-canvas;
}
...
Why: a skin owns a reusable combination without requiring a particular mix of canvas, ink, or edge roles.
...
Why: no component or stylesheet references `primary-skin`, so the utility is dead and only inflates the global stylesheet.
...
Why: the component references `primary-skin`, so the utility has a live consumer and stays.
